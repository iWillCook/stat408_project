...
In this case study we are focusing on the median SWE for each snow telemetry site in Montana from 2016 to 2025. After manually assigning each snow telemetry site in Montana to its respective drainage basin using the HUC6 code, we were able to calculate an average SWE from 1991 to 2020 for each site, and compare the overall average to the yearly average based on the watershed. Doing this we are able to visualize the difference in snow water equivalent not only for each drainage basin or region in Montana, but compare the same basin to itself during different water years.
